...
**Direction of fix (ADR-090, signed):** hobbes edges 3,610 → 3,598 (−12: the twelve `format!`→`fn format` edges left the graph, not the bucket); confirmed 3,592 → 3,592 (0); contradicted 12 → 0 (−12); silent 6 → 6; precision 99.7% → 100% (+0.3); recall 3,593/3,662 → 3,593/3,662 (0). Earlier the same day, H-16 (harness): oracle pairs 4,426 → 3,662 (−764 `.await` poll sites, of which Hobbes had drawn 115 — hits 3,708 → 3,593, −115), recall 83.8% → 98.1% (+14.3), `static→closure` misses 649 → 0.
...
## Regrade after ADR-090 (2026-08-25, later the same day) — the standing grade

Re-ingested at f3cc3eb3 with the macro veto; `~/.hobbes/bench/oracle/dagger-rust/report.txt` is this run (`report.before-w1.txt` beside it keeps the first pass). Head verbatim:
