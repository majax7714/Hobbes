...
| `observed→closure` / `observed→lambda` | Python (trace): a call the interpreter made into a nested function or a lambda | a **named nested `def` is a symbol** and a direct call by its name is drawn (`semantic`; click's `Group.invoke._process_result`) — corrected 2026-09-20 (ADR-146's probe): this row read "nested functions are not graph symbols". What is missed is a nested def reached **as a value** (a decorator factory's inner def applied at `@f(…)` — click 728 rows, ADR-147's; a callback handed to a runner), a lambda (no name, no symbol), and — until 2026-09-20 — a `<genexpr>` frame entry, the compiler's own call, which the key no longer holds (H-36, fixed: it was 590 of this repo's 642 closure misses; keys made before that date still carry the rows) | C-58 |
