...
| RC-3 | Oracle right at a different grain than the binding | H-3, H-5, H-6, H-18, H-19, H-28 | 6 | closed-policy | D-O4, extended 2026-08-27 with the element-access bullet (A-4) and 2026-08-28 with the function-valued-binding (abstract) and `@overload`-anchor bullets. Two sightings against a closed-policy root in one triage: the rule held, its bullet list was short. H-28 (O10, open): a member call's position, the object's start against the member's token. D-O4 has no bullet for it yet. |
| RC-4 | Silence that reads as a result | H-1, H-12, H-26, H-30, H-31 | 5 | closed-policy | RR-6: absence prints as its own state. A-1 landed 2026-08-27 (`no-roots`). H-30 and H-31 (O10, open) violate the rule at the matcher: a call the key holds as `dynamic`, or does not hold at all, lets the line's other calls judge Hobbes' edge. |
...
| RC-8 | One declaration, two spellings — a cross-compilation identity key that is not canonical | H-20, H-29 | 2 | shaped | The key a shard emits must be built from the element model, never from a printed type: annotations, type-variable bounds and generic arguments all differ between a source compilation and the class file another compilation resolves against. Any future oracle that joins facts across separate compiler runs inherits this row. H-29 (O10, open) is the reverse face, and the row's prediction held: two declarations, one key. A class-template pattern's member has no mangled name, and the fallback joins by bare name. Moved from closed-structural to shaped: the guard was Java's key, not the C reader's. |
