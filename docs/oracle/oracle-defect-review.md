...
|---|---|---|---|---|---|
| RC-1 | Cell membership / scope defined in more than one place | H-2, H-10, H-34 | 3 | closed-structural | A-2 (2026-08-27): both Go sides call `edges.Under`/`edges.Excluded`. Per-runtime extractors (TS/py/rust) keep a local predicate — a sighting there reopens this row. |
| RC-2 | Code nobody wrote attributed to a source line | H-13, H-15, H-16, H-21, H-24, H-31, H-36 | 7 | closed-policy | Provenance rule: drop and count by default; reclassify when the dropped class is semantically real (macro→function). H-21 added the converse: a *declaration* the compiler synthesised at a source line a reader can point at (javac's default constructor, at the class line) is kept — dropping it costs real pairs. **H-31 (O10) is the sixth sighting, closed 2026-09-16:** a callee whose qualifier a macro body supplied was keyed on the `#define`'s own line — in fmt another file entirely — so six real calls sat where nobody wrote them and the use site read as having no such call. The rule takes the author's own name token (ADR-113 §3). The root's shape held again: the position of macro-carried code is where this lane keeps going wrong, and the tell was that the call's own *argument* keyed correctly while its callee did not. |
| RC-3 | Oracle right at a different grain than the binding | H-3, H-5, H-6, H-18, H-19, H-28, H-35 | 7 | closed-policy | D-O4, extended 2026-08-27 with the element-access bullet (A-4) and 2026-08-28 with the function-valued-binding (abstract) and `@overload`-anchor bullets. Two sightings against a closed-policy root in one triage: the rule held, its bullet list was short. H-28 (O10): a member call's position, the object's start against the member's token. **Closed 2026-09-16** — D-O4 now carries the bullet it lacked (`bench/oracle/README.md`, "C++ member calls"), and ADR-113 §3 names the rule: a `MemberExpr` has no `loc`, so the site is its `range.end`. The gap this row predicted was real: the bullet list was short, and the sixth sighting is what forced it. |
| RC-4 | Silence that reads as a result | H-1, H-12, H-26, H-30 | 4 | closed-policy | RR-6: absence prints as its own state. A-1 landed 2026-08-27 (`no-roots`). H-30 and H-31 (O10) violated the rule at the matcher: a call the key holds as `dynamic`, or does not hold at all, let the line's other calls judge Hobbes' edge. **H-30 closed 2026-09-16** — §7d and D-O4 carry the rule, and `Grade` buckets such a row `silent`/`line-unresolved`. **H-31 stays open**: the rule cannot reach a call the key holds nowhere, because that line's sites are all resolved, and its `os.cc` half did not reproduce under a probe. **The closure has a price this row should carry:** silencing is indiscriminate — it also hid 6 of C-153's rows, where Hobbes is wrong, and it withdrew the poison check from those lines. A rule that answers "absence is not a verdict" cannot also answer "whose error is this". |
| RC-5 | Ratio quoted with unlabeled over-approximation or mismatched denominator | H-4, H-14, H-22, H-23 | 4 | closed-policy | Split by class, label inflation; numerator and denominator from one index. Log rule 2. H-22 was a violation for a day: the exploratory bucket merged distinct identities; fixed and measured 2026-09-10 (later) — 0 / 3 pairs, no hit moved. H-23 the same hour the identity reached the grader: the Java key's member-bare names folded 2,622 jsoup declarations; qualified, re-merged, positions unchanged. |
...
| RC-8 | One declaration, two spellings — a cross-compilation identity key that is not canonical | H-20, H-29 | 2 | shaped | The key a shard emits must be built from the element model, never from a printed type: annotations, type-variable bounds and generic arguments all differ between a source compilation and the class file another compilation resolves against. Any future oracle that joins facts across separate compiler runs inherits this row. H-29 (O10) is the reverse face, and the row's prediction held: two declarations, one key. A class-template pattern's member has no mangled name, and the fallback joined by bare name. Moved from closed-structural to shaped when it was found, because the guard was Java's key and not the C reader's. **Closed-policy 2026-09-16:** the C reader now spells its fallback `Class::Name`, the same owner-qualified rule H-23 put on javac's key, so both keys that join facts across separate compiler runs carry it and a third sighting is a rule violation rather than a new class. |
| RC-7 | Miss-class taxonomy diverges across languages | H-7, H-33 | 2 | provisional | Patched: local-binding split from closure; TS modes derived from binding shape so Go and TS classes read alike. H-33 (2026-09-19, fixed the same day): the parameter shape never fired — `declKind` tested `isClosure` first; 383 of the TS cells' 1,095 `static→closure` pairs were parameters. Watch for the Rust/Python analogue. |
