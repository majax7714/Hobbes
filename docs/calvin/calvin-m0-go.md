...
**Amendment (2026-09-11, Calvin M0-Gate's orchestrator; D-x — beside the original, which stands as written).** O's session repo held gitleaks' history past each key's parent, and the session policy allows `git log*` / `git show*`. Two of WP-6's five O sessions read their own key commit: `2278a2a97e42` (13 history commands, `git log --oneline --all -20` and `git show 2278a2a:cmd/root.go` among them) and `93acc6e82adb` (`git log --all --grep=…`, `git show 93acc6e`). **These are the two O keys this round read as recalled**: O's verbatim reproduction of gold there came from the clone's history, which the recall scan cannot see, not (only) from training data. Round 1's O rows already do not stand (Max, WP-12); this is a second reason. T is unaffected (T has no exec tool). No O session read another session's transcript or fetched from the network. Fixed in 0.1.20-beta (`harness.run_o` cuts O's repo at the parent; C-124); the record is `docs/calvin/calvin-m0-gate.md` §10.
