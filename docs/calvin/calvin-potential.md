...
**[M0-Go, 2026-09-11] Template v2: the out-degree cap** (`docs/calvin/calvin-m0-go.md` §10, F1). An anchored symbol with more than k = 20 distinct in-repo callees does not expand them. Each callee becomes a round-1 `ANCHOR_CONFIRM` showing its signature line only, and only a confirmed callee becomes a body; an answered one is not asked again. At or under k, callees expand as in v1. k = 20 sits in gitleaks' gap: 99.66% of calling symbols have at most 17 callees, and the next value is 177 (`cmd/generate/config/main.main`'s registry). v2 is opt-in (`build_template(version=2)`); v1 stays the default, and the 28 M0 templates rebuild byte-identically under it. The `hobbes template` CLI builds v1 only. At A2 over the 20 M0-Go keys, round 2's open holes fall from 6,973 to 1,866 and its characters from 35.2M to 3.22M; round 1 grows to 1,581 holes, almost all confirmations. Strict Go coverage goes from 86/7/6/4 to 86/6/6/5: one hunk is lost that v1 reached only through the registry's fan-out (C-104).
...
**[M0-Go, 2026-09-11] Protocol v0.3 supersedes v0.2** (Max's decision on WP-5's first run; `adapter.PROTOCOL_VERSION`, stamped on every exchange and arm-T record). The code has no v0.2 switch. In the run, Haiku sent `patterns` for SIGNATURE, BODY and ANCHOR_CONFIRM in six exchanges on four keys (the replay corrected this to five on three). Those types took no pattern: the repair dropped the pattern and the holes ended unfilled, 7 of 22 calls and 36% of the spend. Under v0.3, a pattern of `"unchanged"` on SIGNATURE or BODY, or `"unchanged"`/`"no"` on ANCHOR_CONFIRM, is accepted on the first pass. Each covered hole is filled as unchanged or no and listed under `by_pattern`. A pattern never rewrites or confirms, and an explicit fill wins. A confirmation refused by pattern counts as a refusal and is not carried to round 2, as with silence (C-105). The validator now reads a refused pattern's holes as `missing`, so a repair can name them. The system prompt and the render are unchanged, so the model is not told these forms are allowed. Replayed with no spend: 4 of the 5 refused-pattern exchanges validate on the first pass, filling 321 holes by pattern and saving 4 repair calls ($0.222 of $0.427).

**[M0-Go, 2026-09-11, later] Protocol v0.4 supersedes v0.3** (WP-7a, after WP-6's run; stamped on every exchange and record; v0.3's pattern reading stands). **The declaration hole:** when T-loop's NULL round-trip meets a new name that a call site writes and nothing declares, it offers one hole per (name, scope), with ids `d1…`. The hole carries the name, the binding directory and the partition files in that directory, and the answer must declare that name in a file of that directory with a body naming it. The declaration joins the template and the call site is re-grounded. Grounder NULL rows now carry `scope`: the Go package directory, plus the type for a typed receiver. **The gutter guard:** a SIGNATURE or BODY fill carrying the render's line-number gutter (two adjacent lines numbered in the render's form, counting up by one) is refused and named in the repair, and the grounder refuses it too (WP-6's D-a). Loop closure is now keyed on (hole, term) rather than (hole, term, class), which had counted a NULL whose class changed as closed and opened at once (WP-7a defect 1; no WP-6 row affected). Replayed with no model: the 11 NULL sites of WP-6's records close with scripted gold declarations, 11 of 11 with no new NULL, and the three gutter fills are refused, 3 of 3. The limits are C-106 to C-108.

**[M0-Go, 2026-09-11, later still] Protocol v0.5 supersedes v0.4, with grounder v2** (WP-9, after WP-8 found every placed declaration written against another project's API; stamped on every exchange and record). **Grounder v2, the world check:** a Go fill's imports must be the standard library (go1.26.5's `go list std` less `internal` and vendor paths, 176 packages, pinned as `GO_STDLIB`), a module the governing `go.mod` requires at the parent, or a directory of the module itself holding Go files; any other import line in an edited range is an `import-outside` NULL. A qualifier on a call, selector or type that no import may bind is an `unimported` NULL, and every doubt abstains. **The v0.5 loop:** a NULL inside a placed declaration's body goes back once as a repair of the same declaration hole, one exchange with no validation repair after it. The declaration hole shows a sibling's form: a function of the same kind in the binding directory (the one the fill calls nearest, else the one with most callers), with its file's package clause and imports, capped at 12 lines and 900 characters. The site record reads the grounder's refused list, so a refused declaration reads `refused` (D-f). Replayed with no model: WP-8's 9 placed declarations each raise at least one NULL (WP-8's "9 of 11 closed" had hidden 0 clean bodies), gold still grounds at 0 NULL and HSR 0, and 78 of 80 round-2 diffs are unchanged. The limits are C-109 to C-114.
...
6b. **The four no-spend fixes** (Max: "do the four no spend fixes before next session"; 2026-09-04 night, after the run): a module anchor as confirmations per symbol, not bodies (§2.1, template v1); a `TEST_EXPECTATION` and a verifier grain for tests that import an edited module (C-93 amended); `calvin.box.policy` allows the read-only toolchain probes (`go version`, `go env`, `go list`, `sh -n`, `bash -n`, `node --version`, `command -v`, `type`, `pwd`) and denies `env`/`printenv` outright (the endpoint key rides as an environment variable); `loop.py --token-budget` stops a session with a reason at a prompt-token ceiling (`harness.O_TOKEN_BUDGET`, 1M, on every arm-O argv; `calvin_probe.py o --token-budget`). Six tests; the hand template and the 28 regenerate under v1. **Exercised with no model 2026-09-04 (later; the record's eighth addendum):** the 28 golds re-verified under the import grain — every verdict identical, `P2F` 0, 2,408 import rows added, the one new `F2F` the box's known environment failure; the four arm-T diffs re-verified unchanged (the `TestProfiles` failures had been selected at file grain already); template v1's step-2 instruments beside v0's (symbol 4% → 3%, outside 89% → 91%, max 333 → 135 holes, 17 confirmations a task); the run's round-1 answers replayed into v1 (`calvin_probe.py replay`): with the gold's symbols confirmed `00e5aee` rebuilds to 844 holes with the three `TestProfiles` tests present at tier `import`, with everything confirmed to 1,226 — the saving is conditional on selective confirmation, and the import tier's test holes are the next cost door.
...
Harness, registered for the next run: the loop's stall discipline (7B defaults) cut O off — fixed by flags, recorded; the box policy denies toolchain probes (`go version`, `go env`, `sh -n`); the 30-turn cap is the cost ceiling O hit three of four times; the planner's tokenizer keeps trailing punctuation. What follows (§9): the anchor-stage residual dominates and the fill-stage NULLs were near-miss and closed — so **the protocol changes (module confirmations, importer tests) and a wider set come before any Calvin model**; M1′ stays first among the models. **The four harness and protocol fixes are made (§8 step 6b, the same night, no spend) and exercised without a model the next day: the import grain is safe on the 28 golds, template v1 asks the missed tests once a symbol is confirmed, and the test hole is now the cost door (step 6b's record). The wider set is held with all spend.**
