...
10. **C++ is supported (0.2.23-beta, ADR-113), and the 2026-09-16
   ...
   surfaced (ADR-117); the knowledge store decodes once (ADR-118); the
   ...
   empty (H-28–H-32 fixed; H-33 found and fixed 2026-09-19).
   ...
   edges no longer hidden), then a lost definition's extent read from
   the file's braces and the facts inside it re-homed (ADR-134, C-145
   narrowed again, 0.2.46-beta: no graded number moves — no key reads a
   caller; fmt's lost callers 1,436 → 188, none wrong). C-164 is
   registered unsurfaced beside it (lane A C++ symbols named by a macro:
...
- **The comparative programme (ADR-101/102) — built 2026-09-09; two
  ...
  every contradiction read: repowise's were function-like macros it
  stores as `function` (C-95's C face). **Then (later):** converter@3
  reads a `#define` at the declared line as `macro` (ADR-101 amended;
  dispatched, `3c41`), the four cells regraded with signed direction
  lines (repowise 1,073/1,075 and 780/780). The tools' venvs are under
  `~/.hobbes/bench/comparative/tools/`, the drivers beside them
  (`run-c-cell.sh`, `regrade-c-cell.sh`). **2026-09-15:** the foreign
  C++ cells, pre-registered first (§10.7), then both tools on fmt and
  args (`run-cpp-cell.sh`, `regrade-cpp-cell.sh`). The triage found two
  converter defects, read at converter@4 (ADR-101 amended), and the
  cells were regraded; CodeGraphContext reads no `.cc`, `.cxx` or
  `.hxx` file, so its args cell graded nothing. `oracle import` +
...
- ~~**Python's decorator and fixture-value calls (Max, 2026-09-20:
  ...
  §10.32); then ADR-148 (0.2.67-beta, 2026-09-21), the optional-parentheses
  factory's guards folded over the site's arguments, 81.2% (§10.33); then
  ADR-149 (0.2.68-beta, 2026-09-22), a factory returning another factory's
  ...
  moniker, now refused (C-170, §10.36). Open from it: route c, lane A drawing
  a bare call to the nested def of that name (the seven sites, flask 5 and
  click 2, are its ceiling on the keys; unmeasured — measure first).
...
- **C's evidence, then its remainder (ADR-108, ADR-109).** C has both
  ...
  - **Then the residue:**
    ...
    - ~~C-133's include path~~ **narrowed 2026-09-14** (0.2.15-beta,
      ...
      derived database was **measured 2026-09-19 and not built**
...
- **ADR-087 follow-ups (host knowledge tools).** ~~(a) `list_blind_spots`
  ...
  name.~~ — **done 2026-09-12 (0.1.23-beta), the first dispatched
  session** (`S-20260912T151945Z-417f`, commit `104c164` by
  `hobbes-dispatch`): both tools take `path` as an alias, and both
  descriptions name the argument. (b) An MCP input-schema rejection never reaches the flight
