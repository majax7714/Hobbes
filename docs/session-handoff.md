...
`a60777f`), each with tests, the register entry moved to its segment's
...
repos. The register reads **68 active / 18 lifted / 2 superseded**, two
...
- **Extraction:** the ten entries lifted (above). Register: 88 entries,
  68 active, 18 lifted (C-86–C-88 from the review still active: the
...
1. Extraction residue the lifts named (no GPU): `tsextract` skips every
   symlink where the Python walks skip only repo-internal ones (C-73's
   residual); Poetry / PDM manifests are not read (C-79's residual); a
   callee that is itself an expression (`handlers[0]()`) is still no
   site (C-80's residual, C-63's shape). date-fns's own re-ingest is
   the real workspace cell — see the BUILDLOG for whether its zones
   came back.
