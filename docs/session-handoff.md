...
2. **Running a session** (`calvin-harness.md` §5):
   ...
   - **Launch a dispatch detached** — `setsid nohup sh -c '… hobbes
     dispatch …; echo "exit $?"' > log 2>&1 &` — never as the
     assistant's background Bash: that harness caps a background
     command at ten minutes and kills it (the BUILDLOG's 2026-09-0x
     note), and a killed dispatch strands the session's containers
     (clean up with `podman rm -f -t 0 hobbes-side-<id>` and `podman
     network rm -f hobbes-int-<id>`). Poll the log in the foreground;
     a background waiter gets killed by the harness's memory policy
     while two sessions run. Two units at 200 turns took 25–28 min each.
...
- **Suites** at 0.2.20-beta:
  ...
  - oracle-lane Go on `main` after `a848`'s merge: 95 pass / 5 skip
    (subtests counted), the C++ end-to-end test included;
