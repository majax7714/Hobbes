...
**§A.4 step 4 — the grid: B4 (λ = 0) and B1, three arms, five seeds, one run each, read at 2,200 and 3,100 (`runs/v2-b4-grid`, thirty cells, $3.38 assumed — B4 $0.18 a cell at 16.3k tokens/s, B1 $0.04 at 81k; `v2-b4-grid-report{,-2200}.md` beside it; every cell carries `typed.json` at both reads).** Mean [min–max] over five seeds; §6.6's gate is over seeds only (one run each — item 3 above):

| group | dense-real 2,200 → **3,100** | sparse 3,100 | conflict → context (`C-only-qa`) 2,200 → **3,100** | sibling share of wrong, near, 3,100 | `UNDEFINED` anywhere (primary / secondary) | loss at the read, 2,200 → 3,100 | probe | NMI vs relation (best layer), 2,200 → 3,100 | max share, training batch |
|---|---|---|---|---|---|---|---|---|---|
| B1/none | 0.07 → **0.65 [0.26–0.82]** | 0.10 | 0.90 → **0.74 [0.49–0.98]** | 0.22 [0.11–0.26] | 0 / 0 | 0.98 → 0.75 | 0.37 | — | — |
| B4/none | 0.09 → **0.66 [0.53–0.79]** | 0.10 | 0.81 → **0.65 [0.36–0.86]** | 0.23 [0.17–0.28] | 0 / 0 | 0.99 → 0.76 | 0.38 | 0.23 → 0.20 [0.06–0.29] | **1.00** |
| B1/phrase | 0.06 → 0.71 [0.56–0.78] | 0.10 | 0.85 → 0.75 [0.21–0.92] | 0.24 | **0.00 / 0.00** | 0.99 → 0.75 | 0.37 | — | — |
| B4/phrase | 0.10 → 0.72 [0.56–0.85] | 0.11 | 0.85 → 0.71 [0.60–0.92] | 0.25 | **0.00 / 0.00** | 0.98 → 0.75 | 0.36 | 0.07 → 0.06 | 1.00 |
| B1/lived+phrase | 0.04 → 0.59 [0.22–0.80] | 0.08 | 0.94 → 0.86 [0.70–0.99] | 0.23 | 0.08 [0–0.39] / 0.01–0.04 | 1.05 → 0.82 | 0.37 | — | — |
| B4/lived+phrase | 0.05 → 0.67 [0.50–0.79] | 0.09 | 0.94 → 0.82 [0.62–0.92] | 0.24 | 0.05 [0–0.27] / 0.05–0.08 | 1.04 → 0.80 | 0.37 | 0.13 → 0.09 | 1.00 |

*Reading it, attributed, in the addendum's order.*

- **The reading phase replicates in both blocks at five seeds** (2,200: dense-real 0.04–0.10, a conflicting context followed 0.81–0.94), and the storing plateau at 3,100 is **seed-variable**: B1/none's dense-real spans 0.26–0.82 and B1/lived+phrase's 0.22–0.80 — the calibration's seed 1 (0.83) sat at the top of its spread, and T_v2's "0.8 from free statements" is met by some seeds at 3,100 and not others. A fact about **T** for Max's T_v2: at this cap the storing phase arrives by the seed.
- **§A.5.4, at five seeds: B4's copy route is weaker at the plateau in every arm, and not separably.** Following at 3,100 is 0.65 vs 0.74 (none), 0.71 vs 0.75 (phrase), 0.82 vs 0.86 (lived+phrase) against spreads of 0.3–0.5; seed 1's 0.36 vs 0.77 was the widest pair. Row 1 (the same curve, shifted) is what stands; the loss delta is +0.05–0.14 at 1,200–1,500 (B4 reaches the reading onset a checkpoint later) and **0.00 [−0.06–0.03] at the plateau** in every arm — typing at λ = 0 costs nothing in loss (§A.5.5).
- **§A.5.2 row 1: the pull is in the input map, not in attention.** With dense-real matched (within 0.01–0.08 of B1 in every arm), the sibling share of wrong answers on absent-near is 0.23–0.25 in B4 against 0.22–0.24 in B1 (diff ≤ 0.02, spread 0.1–0.2); shared-callee pairs across modules sit at cosine 0.72–0.78 against 0.72–0.77 for random cross-module pairs, and **under the best operator the three groups are within 0.02** (0.78–0.85 each) — no relation-conditioned similarity in any cell. Typed routing did not restrict the pull.
- **§A.5.1 at five seeds: mode collapse, total on the training distribution.** At λ = 0 the hard router's usage on a training batch is **one type per layer in every cell at every checkpoint** (max share 1.00, confidence 1.00), with the operators at the identity; on the entity-pair readout two or three types appear in the top layers with NMI against the relation 0.20–0.23 in the `none` arm (purity 0.56 against 0.33) and 0.06–0.13 in the phrase arms; the partition is not the direction (0.04–0.05) and not at the query (purity under the trained and held-out phrasings 0.50–0.61 both, chance-level). Failure-list item 1, measured: **the routing learned is one untyped operator, and the little partition it carries is in layers 5–7 of the `none` arm.**
- **§A.5.3 is not readable at this T: the abstention act is absent.** No block emits `UNDEFINED` in the `phrase` arm at 3,100 — **0.00 on the trained `pair` names themselves**, 0.00 on every other row, in every seed — and in `lived+phrase` 0.05–0.08 with one seed per block at 0.27–0.39 and the rest at 0; the secondary rows read 0.00–0.08 the same way. The `UNDEFINED` pairs (three per `pair` name, one per empty relation of a QA-trained symbol) are seen fourteen times in this regime and the act is not learned; in the memorising regime (77 epochs) B1 and B2 refused every trained pair. So the signal rows carry nothing to track (the answer slot's attention to the name is 0.01–0.06 at its best layer in both blocks, and the one AUC over absent rows, B4/lived+phrase 0.95, is one seed's 0.27 refusal rate against noise). Row 4 of the table: not readable in this cell. Implicates **T** (and **A**: the pairs are a rare token under the reading regime), not **B**.
- **§6.2–6.3:** the probe is at chance (0.36–0.38) in every group of both blocks; MI(act; probed) ≤ 0.01. In the reading regime the class is not in the residual at the query's last token for B4 any more than for B1; the authority question has nothing to bind to.

#### Atlas entry — B4, typed attention (λ = 0, the reading regime T_v2, one run per seed) — circuits, not verbs

*The copy route is two heads:* L2H5 and L3H5 carry the context module to the answer slot (ablating both at the onset → 0.505 / 0.28, the top four → the floor; four random heads cost 0.01–0.2), where B1 spreads the same function over eight heads in layers 2, 4 and 5; at the plateau the same two heads carry part of the lookup too (parametric 0.80 → 0.33 without them). *The lookup is B1's:* the FFNs of layers 2–7 together, dense-real within 0.08 of B1 in every arm, the loss delta 0.00 at the plateau. *The inventory is one identity operator:* on the training distribution one type per layer at every checkpoint (share 1.00), ‖R_k − I‖ ≤ 0.11 there; the residual partition (layers 5–7, `none` arm) tracks the relation at NMI 0.2, is not the direction and not the sentence shape, is not present at the query, and arrives before any fact is stored. *The pull is the input map's:* sibling share 0.23–0.25 against B1's 0.22–0.24 with dense matched; no relation-conditioned similarity under any operator (three pair groups within 0.02). *The copy route is weaker at the plateau in every arm and not separably at five seeds* (0.65 / 0.71 / 0.82 against 0.74 / 0.75 / 0.86). *The pressure decides the route* (seed 1): λ ≥ 0.03 removes the lookup and keeps the copy; λ = 0.01 removes the copy and keeps the lookup; only λ = 0 keeps both. *Countable absence was not reached:* no `UNDEFINED` is emitted by either block at this T, so relation-absence as a computed state has no act to be read against; the signal exists as a number (p_k · ā from the answer slot to the name) and has no dynamic range in a block that does not attend to the name there (0.01–0.06).

**What follows, as §A.7 points.** Branch 3 is the one the results select: **collapse at every λ that learns** — at λ = 0 the router is one identity operator on the training distribution, and every pressure that spreads it removes the lookup route — so *typing has to be given, not learned, at this scale*, which is what Hobbes does; the next block is **B4-given** (the inventory fixed to the world's three relations, the router replaced by the relation the pair is in, read from the world) to ask §A.5.2 and §A.5.3 with the discovery question set aside. Before either question can be read, though, **the regime must carry the abstention act**: at T_v2 no block learns `UNDEFINED` from fourteen exposures, so §A.5.3 needs the memorising T (3,500 steps, where v0 and v1 read refusal) or a computed target (`NO_EDGE`, §A.7 branch 2) — and that choice is the design's, with T_v2 itself (the seed-variable plateau above). The second run of the grid (§6.6's union, ~$3.8) would firm the copy-route difference into separable or not; nothing in the tables suggests it would separate.

*Cost.* The grid $3.38; **this item $4.71 assumed** (the §A.1 cells, K = 1, the profiles, five sweep cells, three re-reads, the GPU checks, the grid), under the addendum's $8 with one run per seed; **$22.28 assumed to date** against the $25 ceiling. Modal's bill is the number.
