...
> **Amended 2026-09-03 (review item 4, second cell record).** Measured, the
> abstention this family trains is *less* than one instruction buys: the
> base model with the held-out card and the sentence "if the symbol is
> not listed, say it is not defined at this SHA; do not guess a file"
> refuses every distractor (false acceptance 0.90 → 0.00) at a cost of
> 0.06 on the has-truth families, where the 300-step adapter reaches
> 0.22 — and under the same instruction the adapter's 0.22 does not move
> while its true answers collapse (defines 1.00 → 0.33). The family stays
> in the corpus (it is what makes the weights refuse without a prompt),
> but the rationale is now "instruct for abstention where a prompt is
> available; mid-train for it where none is", not "mid-train for it".
