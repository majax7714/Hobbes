...
- **Cannot tell you:** anything about the losing file — a repo-root
  ...
  (V2.M7), then Java (ADR-096), then C (ADR-108), then the pack layer's
  nodes, `tf:` among them, last (V2.M4).
...
- **Bites at:** the graph's completeness, by an accident of pipeline order.
  The one real shape named so far is cgo's `foo.go` beside `foo.c`, which
  both derive `foo` (C-132).
