...
The table is current to **2026-09-10**: every compiler-graded cell
stands at 100% precision-against-oracle except quic-go (99.6% lower
bound, every contradiction oracle-grain, 0 hobbes-wrong) — ajv's three
...
| **Oracle-grading lane** (ADR-089) | built, both phases run, **reviewed 2026-08-27** | `bench/oracle`: the call graph graded against RTA / `tsc` / the interpreter / rustc's MIR on this repo, kbet, rust_proj, dagger and the seven-repo loop; every compiler-graded cell at 100% precision but quic-go's 99.6% lower bound, all 15 contradictions the oracle's grain (§3.8); its own defect log reviewed (`oracle-defect-review.md`) |
