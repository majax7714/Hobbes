...
| **JavaScript** | the TypeScript row's five graded cells are TypeScript programs, and none of their 15,167 confirmed edges touches a JavaScript file (C-165) | **No graded cell of its own (ADR-140, measured 2026-09-19).** A `.js`, `.jsx`, `.mjs` or `.cjs` file goes through TypeScript's two lanes under `allowJs` (`checkJs` off): lane A's ts-morph project and lane B's scip-typescript run, with a generated config mirroring lane A's default project where the repo gives none (`tsextract/extract.mjs`, `scipsource._generated_tsconfig`). So a JavaScript repo ingests and draws edges, but no key has judged one: on the TypeScript row's five cells (kbet, ajv, cheerio, zod, hono; the 2026-09-09/10 reports) all 15,167 confirmed edges sit between TypeScript files, and the 27 edges drawn with a JavaScript file at either end are all `silent` — outside the program the zone's `tsc` loaded. The oracle itself (`bench/oracle/ts/tsc-oracle.mjs`) refuses a zone with no `tsconfig.json`, so a plain JavaScript repo cannot be graded until it can (ADR-140's step 3). Until a JavaScript cell is graded this row claims nothing, and the ingest summary, the surface's badge and `list_blind_spots` say so (`verification_base`, depth `unverified`). Neither lane reads a `jsconfig.json` — zones key on `tsconfig.json` alone — and since 0.2.54-beta the ingest says so per file (C-166; on Preact 120 of 22,804 sites resolve differently under its jsconfig, the key alone) |
...
multiple repos of different shapes. **JavaScript has no repo of its own**
(C-165, ADR-140): it shares TypeScript's lanes, and every TS/JS cell
turned out to be a TypeScript program, so its row claims nothing until a
JavaScript cell is graded. **Go's hand-verified evidence base was one repo, Hobbes's own** — a
...
**Hobbes 0.2.54-beta** (2026-09-19, ADR-103; beta: graded, not stable; the latest tag `v0.2.10-beta`, the one before it `v0.1.8-beta`; 0.1.9-beta to 0.2.9-beta and 0.2.11-beta to 0.2.54-beta untagged; `CHANGELOG.md` is the
