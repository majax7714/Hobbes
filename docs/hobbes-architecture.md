...
**Hobbes 0.2.11-beta** (2026-09-13, ADR-103; beta: graded, not stable; the latest tag `v0.2.10-beta`, the one before it `v0.1.8-beta`; 0.1.9-beta to 0.2.9-beta and 0.2.11-beta untagged; `CHANGELOG.md` is the
...
| **Calvin harness** (ADR-107; `calvin/calvin-harness.md`) | built 2026-09-12, 0.1.21-beta (the 0.2.0-beta minor); validation by use under way, at 40 sessions (Max, 2026-09-13): thirteen session logs through 0.2.11-beta (from a rejected token and the `path` alias to C's lane A and oracle, the progress hook, ADR-111's veto, `list_blind_spots`' directory rollup, C-139's lift and the session's containment); the progress hook (0.2.6-beta), the dispatch box (0.2.7-beta) and a session mounting only its own dir, with `find`'s executing forms and `xargs` as questions (0.2.11-beta, ADR-107's 2026-09-13 amendment) since | `hobbes dispatch`: Claude Code in `hobbes-session` behind the egress allowlist (`hobbes-session --egress`, `hobbes-proxy egress`), `hobbes gate` with a derived map and `hobbes verify` on its diff, one log file per session under `docs/calvin/sessions/`; the keyed rounds (M0, M0-Go, M0-Gate) closed as an approach, their records history; C-41 narrowed, C-124 superseded, C-125–C-129 (C-125 narrowed by the hook), C-140 (a doer can still reach its own session's records) |
