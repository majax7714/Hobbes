...
- **P12 — A Hobbes test decomposes, or it is not a Hobbes test.** Hobbes
  breaks work up and derives context *in order to* shrink the window a
  large task needs, sharpen what an agent recalls, and specialise the
  work across sandboxed roles. A run is a Hobbes arm only if a planner
  defined the units, the task was split across more than one single-use
  agent with distinct roles, and **every implementer's window was smaller
  than the task's**. A run that fails any of the three is *model +
  prompt* and is reported under that label — whatever its score. The
  rule exists because the decomposition slipped out of the programme in
...
package's files. A third rule came from quic-go (ADR-098, 2026-09-02):
**a package may declare one name once per build configuration**
(`//go:build linux` / `_darwin.go` / `!go1.25` alternates, and the
external `_test` package sharing the directory), so the fallback keeps
every declaration, considers only the caller's own package, and where
constraints split the name resolves only when exactly one declaration's
`build_constraint` — the `//go:build` line as written plus the
filename's GOOS/GOARCH, compared, never evaluated — equals the caller's;
otherwise it abstains and the tail says `build-tag-set`. The semantic
index answers for one configuration (the image's linux), and a
`scip-go` record names the constrained files it left dark (C-71).
...
declared in every file — is unattributed rather than guessed), C-170 (scip-python gives a
method's same-named nested defs one moniker, so a Python moniker one file defines at several
lines is no lane B answer, as in C++, rather than its first line; ADR-150, 0.2.70-beta),
...
missing helper; one that ran out of memory, or was killed, says so and
names C-150. **The helper's decode streams the index** (ADR-115,
0.2.25-beta): it walks SCIP's wire format one document at a time and
...
reserved for ids lane B would have to invent was never implemented —
no code defines it, because nothing ever needed it.
...
**The external veto (ADR-111).** Lane A's fallback is dropped at a site
whose `(file, line, name)` carries a lane B reference to a declaration
outside the repo. Lane B has already answered "not in this repo", and a
same-named repo function there is lane A's guess, not the build's target
(C-138: a portability shim in a dead `#if` arm, drawn over libc's).
- **Only an outside reference vetoes.** The helper marks an external
  reference `in_repo` when its moniker has any in-repo definition: one
  ambiguous across files (C-28), or one of a kind the graph drops.
  `join_cross_unit` marks the monikers sibling units define ambiguously.
- **Coverage is unchanged.** The site was already counted `external`.
...
**A symbol lane B declared (ADR-129, 0.2.41-beta).** Symbols are lane
A's, with one named exception. Where lane A's parse of a C or C++ file
had ERROR nodes and lost a definition (a declaration spelled through
macros is not C++ to tree-sitter, C-145), lane B's own definition row —
file, line, kind and moniker, which the helper already carried — becomes
the symbol, after the join and before the projection, so the calls the
indexer resolved there draw instead of falling `below-floor`. The
symbol says who declared it (`declared_by: "scip"`), is spelled as the
compiler spells it, and is minted only under lane A's own contract: a function,
method or type, defined with a body the file's text shows, not inside a
function, not unnamed, and nowhere lane A already has the name nearby.
Every refusal is counted in `graph.json`'s `minted` block. Nothing lane
A decides sees a minted symbol — the fallback, the vetoes and lane
agreement are settled before it exists — and with no lane B nothing is
minted (P6). Measured first and fitted on fmt, with args held out
(`oracle-grading.md` §10.13): fmt 3,269 → 6,510 confirmed call edges at
0 contradicted, recall 14.5% → 29.1%.

**Its extent (ADR-134, 0.2.46-beta).** The index carries no extent
(scip-clang emits no `enclosing_range`), so a minted **function or
method** gets one from the only evidence there is: the file's own text,
comments and literals blanked, the first `{` at parenthesis depth 0
matched to its `}`. `end_line` becomes that line and the symbol says
`extent: "braces"`. It is refused — the symbol stays a target, `end_line`
its line — where a preprocessor conditional sits in the body, where the
match runs off the file, and where any other function or method starts
inside it (a macro-generated definition has no brace of its own, and the
match would borrow the next function's). A minted type always stays a
line. After the mint and before the projection, a `calls` or `uses` fact
written inside a read extent is re-homed to it when its scope is the
module (lane A's file-scope sites carry the module's id) or a lane A
symbol that starts before the extent; nothing else reads an extent, and
the counts are `graph.json`'s `minted.extents` (`read`, `refused` by
reason, `rehomed`). **No key grades a caller**, so this rule's check is
not a grade: the key's own caller names, read before and after by a
...
(ADR-145, 0.2.63-beta).** At `runner.invoke(cli)` the index names the
parameter and emits nothing at `invoke`: scip-python does not type an
unannotated parameter. But both hops around the silence are already in
the graph — the `uses` injection above, and the `semantic` `calls` edge
the index drew from the fixture to the class at its `return
CliRunner()`. Lane A's Python walk now keeps a definition's one valued
`return`/`yield` when it is `C(…)` on a bare name, and the parameters
its own body rebinds; `fixtures.value_calls` draws `p.m(…)` → `C.m` as a
`calls` edge, tier `syntactic`, evidence `via: fixture-value`, appended
after the projection like the injections. A construction fixes the
runtime class exactly, which is why it is read and a return annotation
is not. Refused and counted (`fixtures.value_calls.refused`): a rebound
parameter, a value that is not a construction, no semantic class edge at
that line (so nothing without lane B), `m` not one `def` in the class's
own body — an inherited method included — a property, and a pair the
join already drew. Resolution coverage is not moved: the join did not
resolve the site. click: 382 drawn, 381 confirmed by its trace key, 0
contradicted, recall 38.2% → 46.5% (`oracle-grading.md` §10.29); six
other-language cells row-identical.

**The value through a local, and an inherited method (ADR-145 amended,
0.2.69-beta).** flask's `app` fixture binds `app = Flask(…)`, configures
it, and `return app`; most of what a test calls on it is `Scaffold`'s or
`App`'s. Lane A's walk now also reads a returned bare name bound exactly
once, by a top-level `x = C(…)` before the return, with no `nonlocal` /
`global` of it and no nested def of its name (`Symbol.value_local`); the
class edge is looked for at that assignment. Where `C` has no `def m`,
`fixtures.value_calls` walks up single bases the index names on each
class's own `class` line (any `semantic` edge type — in flask `Flask →
App` is a `uses` edge and no `implements` one), taking the first `def`
Python would find. It refuses and counts `patched` (the test or the
fixture stored onto `p.m`, `p.__class__`, or passed `p` to a `setattr`;
`Symbol.patched`), `class-binds` (a class-body binding of `m` other than
a `def`, at any class on the walk; `Symbol.binds`), `multiple-bases`
and `base-unnamed`. flask 1,121 → 1,521 confirmed on its new key (41.5%
→ 56.4%), 400 added, 329 inherited, 0 contradicted; click unchanged
(§10.35).

**A decorator is a call of what it names (ADR-146, 0.2.65-beta).** Until
then lane A's Python walk skipped a decorator's expression, so
`@click.option("--x")` — a direct call of a declared function — had no
site for the join to pair the index's occurrence with, and no edge
(C-169, registered and lifted the same day; on click 1,652 of 2,459
missed pairs sat on a decorator line). Now the expression is walked as
any expression is, scoped to the **enclosing** definition or the module,
and a bare `@name` / `@a.b` records the application the language
defines, at the terminal identifier. Lane A adds sites and nothing else:
an edge appears where the index names a repo symbol, `semantic`, and an
untyped `@app.route` stays in the tail. What a decorator *returns* is
not followed (the factory's inner def is a function value, C-58). The
digest the packs read takes the decorator's first non-comment child, so
a trailing comment no longer blinds it. click 2,136 → 3,052 confirmed
(46.5% → 66.4%), the same 18 suspects, poison PASS; flask +166 and
attrs +120 rows, all `semantic`, none lost (§10.30).

**A decorator factory's application is a call of the def it returns
(ADR-147, 0.2.66-beta).** `@click.option("--x")` then applies what
`option` returned, and no token spells that call, so it is appended
after the projection like ADR-145's: `decorators.factory_calls` reads
the `semantic` edge the index drew to the factory at the decorator's
line, looks up the factory's one body (past `@overload` stubs), and
draws `calls` to `f.<locals>.g`, `syntactic`, `via: decorator-factory`,
only where the body is undecorated, not async, not a generator, and
**every** `return` in it is the one nested `def` bound once — the strict
wording, so the claim holds on every path. The caller is the edge's own
`from`, a module node at the top level. Refusals are counted in
`graph["decorators"]` and on the ingest's `decorators:` line (click: 491
factories with another return path, the loose wording measured and not
taken). click 3,052 → 3,356 confirmed (66.4% → 73.0%), 0 contradicted,
the probe's figures exactly (§10.31). C-58 keeps every other function
value.

**A factory's guards, folded over the site's own arguments (ADR-148,
0.2.67-beta).** ADR-147 refuses a factory with any other return; click's
`command` is the optional-parentheses idiom (`return decorator(func)` for a
bare `@command`, `return decorator` for `@command()`), and the path is
decided by what the site writes. Lane A digests each call-form decorator's
literal arguments (`Decorator.bound`) and, for a factory ADR-147 cannot
settle, its signature and own body as a small program (`Symbol.inner_fold`:
literal assigns, `if`/`elif`/`else` tests, `return g` or other, `raise`,
and every other statement as the names it binds — a loop, `with`, `try` or
`match` keeps each return in it reachable). `decorators.fold_guards` binds
the site's arguments (unfilled parameters at their literal defaults,
`*args` at `()`) and folds three-valued — `is None`, `is not None`,
`callable()` on a literal, `not`, `and`/`or` — taking both arms of an
unknown test; it draws `calls` to `g`, `syntactic`, `via:
decorator-factory-folded`, only where every reachable return is `return g`.
A method factory at a site passing a positional is refused
(`method-positional`: lane A cannot tell `@obj.f(x)` from `@Cls.f(x)`); a
reachable other return is `guard-unknown`. click 3,356 → 3,703 confirmed
(73.6% → 81.2% on `click-py-r3`), 397 folded, 3 new suspects — tests where
a lower decorator raises before `command()`'s `decorator` is applied, the
code as written — poison PASS, the probe's rows exactly (§10.33). attrs 18
folded (`@attr.s(…)`).

**A factory that returns another factory's call (ADR-149, 0.2.68-beta).**
click's `group()` holds no nested def and ends `return command(name, cls,
**attrs)`. Lane A digests such a body (`Symbol.chain_fold`, where neither
`returns_inner` nor `inner_fold` is set) as ADR-148's program with each
`return <call>` recording its callee chain, its terminal line and its
arguments (`Chain`: operands, a `*x` index, a `**x`). `decorators._chained`
folds the outer factory over the site's arguments keeping each reachable
return's environment; every reachable return must be a call the index names
**from the factory** at that line to one function or method `G`, the same
at each; `G` must settle (ADR-147) or fold to its nested def over the
forwarded arguments (`_forward`: a name read in the outer environment, a
`*x` unknown from its index on, a `**x` leaving every unbound parameter
unknown rather than defaulted). Drawn `calls` to `G.<g>`, `syntactic`, `via:
decorator-factory-chained`, one level only; refused `chain-guard-unknown`,
`chain-unresolved`, `chain-inner`. click 3,703 → 3,755 confirmed (81.2% →
82.3% on `click-py-r3`), 67 chained, 0 new suspects, poison PASS (§10.34).
...
can name it, C-63 surfaced 2026-09-05), a member call on a union-typed
receiver whose members resolve the member differently (`union-member` —
the TS helper types the receiver and abstains, the join vetoes lane B's
first-member pick, ADR-104, C-97; TS/JS only), joined since
...
**Built at V2.M4 (ADR-035), grown since.** Eight packs — `http-python`
(FastAPI/Flask decorator routes), `cli-python` (`[project.scripts]`),
`http-ts` (Express/Nest), `http-go`, `terraform` (the whole HCL layer and
its joins), and the three CLI packs C-14's lift added (`cli-ts`, `cli-go`,
`cli-rust`) — and the graph builder contains no framework knowledge at
all. All eight declare `syntactic`: each reads structure with no semantic
provider behind it. The registry of record is
`extract/packs/__init__.py`.
...
walk, a bounded mechanical job with seven worked examples now (`pysource`,
`tssource`, `gosource`, `rustsource`, `javasource`, `csource`, `cppsource`, and HCL's). The claim that was wrong
...
**C (ADR-108) was the seventh walk, and the first wired with no indexer
...
There are zero lines in the builder, the join or the schema. C's §3.8
row landed with ADR-110's oracle cells (2026-09-12, 0.2.5-beta); until then it was wired, not supported. It is also the first
...
**C++ (ADR-113) was the eighth walk, built in two units in parallel
with its oracle.** `cppsource.py` on `csource`'s contract, C's rules by
import where they are C's (includes, the fallback ranks, the record
shapes); a `.h` claimed by its includers (C-142); the `cpp` tail bucket
with a row-language override — a coverage row may carry `language`,
and both the tail and the proxy's copies prefer it over the extension,
the drift test's new clause. Zero lines in the builder, the join or the
schema. Its lane B is C's (unit 2, 0.2.19-beta). C and C++ files go
to scip-clang as one set, and a root holding C++ runs as the helper's
`cpp` language: C's spec and decode, plus one rule, a constructor over
its class at one site. Every overload is a symbol (`~n`, Java's rule),
and a construction whose constructor is implicit falls to the
projection's calls-to-type guard. Its oracle O10 is the C oracle's C++
face (§7d). Its cells found two rules the decode and the join needed
for C++ alone (ADR-113 §2's third amendment, 0.2.22-beta):
- a call site whose references name several overloads has no lane B
  answer, rather than the first line (C-151);
- a C++ file lane B indexed draws no fallback edge where lane B
  answered nothing (C-152). The fallback still feeds lane agreement.

A third, from C-153 (ADR-125, 0.2.37-beta): a lane B call answer whose
owner is an explicit full specialisation (`template <>`) other than the
one the call's qualifier names (`X<A>::f` answered with `X<B>::f`) is
contradicted by the source text, and draws no edge; the site is tailed
`qualifier-mismatch`.

A fourth, from the same entry (ADR-130, 0.2.41-beta): a lane B call
answer written with **more arguments than the target's declarator can
take** draws no edge, tailed `arity-mismatch`. Lane A counts the written
arguments at the site and the parameters at the definition (the mint
reads a lost definition's from its tokens); only "too many" fires — a
default argument lives on a declaration elsewhere — and only where both
counts were read. It exists because ADR-129's mint surfaced 35 wrong
single candidates on fmt that no key can judge; it withholds those, five
that were already standing, and no confirmed edge (§10.14).

**An operator applied by symbol (ADR-131, 0.2.42-beta).** `a + b` names
no callee, and a built-in `+` is no call, so an operator is never a site:
lane A records the operator **token** — line, column, spelling, and
whether it sits under a `template_declaration` — packed one integer each
and read by nothing but the join (ScummVM has about 3.2 million of them
beside 1.53 million call sites). A lane B reference no call site claimed,
named `operator` + that spelling at exactly that position, is a semantic
`calls` fact where the token is **outside a template**, its caller the
enclosing symbol; everywhere else it is the `uses` edge it was. Inside a
template scip-clang answers a dependent operator with its single by-name
candidate at the same arity (C-153), where no key can judge it and no
source text contradicts it: measured first, every unjudged row a naive
rule added was there and about 100 of 161 read wrong, so no call is drawn
there and the ingest counts them (`graph.json`'s `operators` block; C-146
narrowed, §10.15). **Since 0.2.43-beta (ADR-131 amended) such a reference
draws no `uses` edge either:** a `uses` no key grades is believed as a
dependency, and the candidate is the same wrong one. fmt lost 156 `uses`
symbol edges and args 24, no graded row moved, and the one module edge
that went on each cell was wrong (integer `n * 10` read as `fp`'s
`operator*`; a library header's `ss >> x` read as its test header's
`operator>>`). Every other unclaimed reference is the `uses` edge it was
(C-153 narrowed a third time, §10.16).

**A construction (ADR-132, 0.2.44-beta).** `T x(args);`, `T x{…};`,
`T x;`, a member initialiser, a braced argument, `T{…}`, `new T(…)` and
a defaulted parameter name no callee either, and `int x(3);` is the same
syntax and no call, so lane A records construction **tokens** beside the
operator ones — the declared name (in a declaration that names a type
and sits directly in a block or at namespace scope), the member's name,
the `{` of a braced argument or return, the `=` of a default, the type's
start — packed, never sites. A lane B reference no call site claimed,
**onto a constructor** (its definition row's moniker ends `T#T(…)`,
`minted.constructor_lines`), at exactly such a token and **outside a
template**, is a semantic `calls` fact; inside one it stays the `uses`
edge it was and is counted (`graph.json`'s `constructions` block).
Measured first: fmt +92 confirmed at 0 contradicted, the 24 rows the key
could not judge read right by hand; args, held out, +369, all confirmed,
62.5% → 72.9%. 80% of fmt's missed constructions sit at a macro
invocation's name (C-131) and are not this rule's; an implicit
conversion has no token and draws nothing; a type named through a
using-declaration lost its constructor's reference to the join's
by-name claim (19 rows on fmt) until the claim went by position at
0.2.45-beta (ADR-133, C-163 lifted; §10.18). C-162 registers the rest
(§10.17).

**A TypeScript or JavaScript construction (ADR-142, 0.2.57-beta).** The
same shape, a language later, and the reason is the opposite one: `new
X()` *does* name a callee, but scip-typescript answers at that token
with `X#`<constructor>`()` — a constructor, whose own line starts no
lane A symbol (C-58's floor), so the reference fell below it and the
edge was nobody's. Lane A records the `new` expression's callee terminal
identifier as a construction token (facts v6), never a site, for
ADR-132's reason: lane A can say a construction was written, never what
was constructed. A lane B resolution no call site claimed, at **exactly**
such a token, is drawn `calls` at semantic tier — **onto a constructor**,
to the class that declares it (the definition row whose moniker is the
constructor's minus its last descriptor); **onto anything that is not a
class**, an ES5 `function User(…)` or the variable bound to it, to that
definition. **Onto a class, nothing is drawn** and the refusal is
counted (`ts_named_class` beside `ts_drawn`): the written class declares
no constructor there, so the one that runs is a base's — which is what
the key names, and what made the naive rule 55 contradicted rows in the
probe. Measured first (`oracle-grading.md` §10.22, §10.11): xmpp.js
552 → 676 confirmed at 0 contradicted, 66.4% → 81.2%; ajv 1,410 →
1,499; hono 768 → 833; zod 9,731 → 9,872; Express 992 → 998; 100%
precision on every cell. C-168 registers the rest — `super(…)`, where
the index emits no occurrence at all; a JSX tag of a class component,
...
scip-typescript at the member token with the exported literal's *property*
(`` `m.js`/f1: ``), a `meta` symbol no definition is kept for, so the site was
an in-repo external reference and the function got none. The index tells the
literals of a file apart itself, and a shorthand is a property definition and
a reference to the function **at one exact range**; the helper's decode reads
that coincidence and nothing else: a property with exactly one definition
occurrence, at whose range exactly one defined symbol is referenced, is an
alias of it, and a reference to the property is filed as a reference to that
symbol. Both hops are the index's, so the edge is `semantic` (Max). A value
property (`delta: alpha`), a method written in the literal, a property defined
twice and a shorthand naming something the index does not define are refused.
Measured first (`oracle-grading.md` §10.28): cue 881 → 1,005 confirmed, 54.3% →
61.8%; xmpp.js 676 → 705, 81.2% → 84.6%; 0 contradicted; seven other TS/JS
cells and seven cells of the other languages row-identical.

Its §3.8 row landed on the regraded cells (0.2.23-beta), and C++ is
*supported* as far as that row reaches.
...
|---|---|---|
| **Python** | this repo (dogfood, continuous — **trace-graded**, twice); **pallets/click**, **pallets/flask** (trace-graded); + six SWE-bench repos at span/declaration grain (astropy, django, scikit-learn, sphinx, sympy, xarray) | **Trace-graded (oracle lane O6):** this repo's Python zone under its own suite — 2026-08-25: 3,291/3,490 call edges confirmed by the interpreter, 6 wrong (all syntactic), then 3,302/3,495 and 0 wrong on the executed slice after ADR-090 the same day; 2026-08-28 (contained, ADR-092): 3,311 confirmed, 5 suspect, **recall-against-executed 86.1%, 97% on named declarations** (closures and lambdas draw no edge — C-58); a coverage-limited claim, never precision (C-60; `docs/oracle/cells/hobbes-py-2026-08-2{5,8}.md`). **click** (2026-08-27/28): 1,699 confirmed, 18 suspect (1.0%; 16 are C-60's override/monkeypatch asymmetry), recall 37.0% — the decorator factories' inner closures (C-58); the `@overload` grain fixed on the oracle's side (H-19); **3,755 confirmed, 21 suspect, recall 82.3% at 0.2.68-beta on the standing key `click-py-r3`** — 52 of them a factory's application through a second factory's call (`group()` → `command`, the `*_option` family → `option`), 0 new suspects (ADR-149; §10.34); 3,703 and 81.2% at 0.2.67-beta — 347 of them an optional-parentheses factory's application, its guards folded over the site's own arguments; the 3 new suspects are tests where a lower decorator raises before the application runs (ADR-148; §10.33); 3,356 and 73.0% at 0.2.66-beta (73.6% on `click-py-r3`, §10.32) — 304 of them a decorator factory's application, drawn `syntactic` where every return is the one nested def (ADR-147; §10.31); 3,052 and 66.4% at 0.2.65-beta, 916 of those calls a decorator makes, which lane A did not record as sites before (ADR-146, C-169 lifted; `oracle-grading.md` §10.30); 2,136 and 46.5% at 0.2.63-beta, 381 of those `runner.*` calls on the value the `runner` fixture constructs (ADR-145, `syntactic`; 1,755 and 38.2% before; §10.29). What was left at 0.2.66-beta was 859 closure pairs — a factory with another return path (`command`), a callback handed to a runner — 215 methods, 81 lambdas; at 0.2.67-beta 512 closure pairs, most of them callbacks reached through attributes and parameters (values, C-58), `group()`'s chain through `command(…)`, and a decorator held in a variable. **The key was regenerated the same day (H-36, the oracle's defect: a `<genexpr>` frame entry keyed as a call): 4,561 pairs, recall 73.6%, nothing else moved; and this repo traced fresh at 0.2.66-beta reads 8,162 confirmed, 26 suspect, recall 94.8% (8,162/8,612) — 85.6% under the pre-fix extractor on the same tree, all 923 rows of the difference `<genexpr>` (§10.32).** **flask** (2026-09-22, keyed for ADR-145's amendment; contained, 494 passed both runs; `docs/oracle/cells/flask-py-2026-09-22.md`): 1,121 confirmed, 18 suspect, recall 41.5% at 0.2.68-beta — **3 of the suspects Hobbes-wrong** (a nested def resolved to a sibling test method's same-named nested def), 15 C-60's override asymmetry; 1,521 and 56.4% at 0.2.69-beta, 400 calls on the value the `app` fixture binds to a local, 329 of them a base's method, 0 contradicted (ADR-145 amended; §10.35); **1,519 and 56.3%, 15 suspect, at 0.2.70-beta** — the 3 wrong edges were scip-python's one moniker for a method's same-named nested defs, filed under the first def, and are refused with the two right only by order (ADR-150, C-170; §10.36; click 3,755 → 3,754, its one wrong edge of the shape gone). Lane agreement 1,789 sites / 0 disagreements; 10/10 sampled narrative claims resolve (M5). The derivation programme (2026-08) hand-checked **spans and declaration sites** on the six benchmark repos (24k–608k detected sites each — every one checked landed exactly); **call edges were not separately sampled there**, so those repos extend this row at span/declaration grain only (evidence log) |
| **TypeScript** | kbet (real Vite+React app); **ajv-validator/ajv**, **cheeriojs/cheerio** (2026-08-27/28); this repo's `web/` (lane agreement only) | **Compiler-graded (oracle lane O3, the zone's own `tsc`):** kbet 630/630 call edges confirmed (2026-08-25), recall 633/637 on declared callees and 633/1,529 over every resolved site (local bindings, closures and interface members draw no edge — C-58, C-32); **ajv** 1,375/1,378 on 2026-08-27, **1,410/1,410 on the 2026-09-09 regrade** (contained) — the 3 were a member call on a union-typed receiver drawn to the first member's override (`static→union-member`); since ADR-104 lane A abstains there and the join vetoes lane B, ten sites in the tail as `union-member` (C-97), recall 63.5%; **1,499/1,499 at 0.2.57-beta, recall 67.5%** (ADR-142: 89 constructions of a class through its own constructor, 6 refused where the index named a class); **cheerio** (unmoved by ADR-142: the index names nothing at any of its 63 `new` tokens) 2,102/2,102 on 2026-08-28 with 44 `abstract` (calls through `const x = factory(..)`, H-18), **2,628/2,628 on the 2026-09-10 regrade** (0.1.7-beta; recall 45.1%, and 68.7% with the oracle's overload grain collapsed to one pair per site and callee — every function-declaration target on the cell drawn, 1,911/1,911; what is left is the specs' `$` bindings and closures below the symbol floor, C-32/C-58; the callee-shape bucket that priced it found C-100, `docs/oracle/oracle-misses.md`). A call through `obj[key]()` is not a site at all (C-63). **zod 9,731/9,731 (recall 45.1% over every resolved site; pnpm not provisioned, C-23) — **9,872/9,872 at 0.2.57-beta, recall 45.8%** (ADR-142: 142 constructions drawn, 192 refused at a class that declares none, the largest such cell) — and hono `tsconfig.build.json` 767/774, 767/768 on the 2026-09-09 regrade, **768/768 on the 2026-09-10 regrade** (recall 55.2%; the 7 were ajv's union-member shape — six abstained by ADR-104, the seventh once C-98's lift gave lane A the referenced project's options to type the receiver with); **833/833 at 0.2.57-beta, recall 59.7%** (ADR-142)** — repowise-bench's pinned draws under our key (2026-09-09). Contained cells (ADR-092): cheerio, zod, hono; kbet and ajv are host-run records. Lane agreement 359 sites / 0; 10/10 test mappings (M6); the V2.M3 20/20 hand-check retired 2026-08-25 |
| **JavaScript** | **expressjs/express** (CommonJS); **preactjs/preact** (ESM, JSDoc, JSX; its root zone); **xmppjs/xmpp.js**, **cypress-io/github-action** and **Blueturboguy07/cue** — drawn at random (2026-09-19/20); two of five graded with their dependency tree | **Compiler-graded against the TS oracle with `--no-tsconfig` (the program the ingest generates for files no `tsconfig.json` claims: `allowJs`, ESNext, Bundler; ADR-140, `oracle-grading.md` §10.22), in the image with no network, 0.2.54-beta.** **Express 998/998 (100%)** at 0.2.57-beta, recall 65.7% (998/1,520) — 6 of them constructions of an ES5 constructor function (ADR-142); 992/992 and 65.3% at 0.2.56-beta, 289 semantic and 703 syntactic: the tests' `express()` reaches `createApplication` through `index.js`'s `module.exports = require('./lib/express')`, which lane A follows and the index does not name, so those 652 sites are drawn syntactic (ADR-141, C-167; 340/340 and 22.4% before); calls through parameters (mocha's `done`) 352; functions assigned to a property are below the symbol floor (46, C-9/C-58). **Preact 2,447/2,447 (100%)**, recall 28.6% (2,633/9,211): graded over its root zone, the three `tsconfig.json` subtrees read `not-loaded`; its first pass read 1,223 contradicted, every one the oracle's (H-34, an in-repo `.d.ts` keyed external; H-35, a JSDoc `@type` keyed as the annotation), fixed and regraded; 6,300 of its misses are in test files (in-body closures, calls through the interface members its `.d.ts` declares, hook setters in locals). **xmpp.js 705/705 (100%)**, recall **84.6%** (711/840) at 0.2.62-beta — 29 of them calls through `export default { … }`'s shorthand properties (ADR-144; 676/676 and 81.2% before), 124 constructions (552/552 and 66.4% before ADR-142); its own lockfile is out of sync with its manifest, so `npm ci` refused and no `@xmpp/*` import resolves in either program (0 cross-package pairs). **A `new F()` is drawn `calls` where the index names the constructor at the token** (ADR-142): to the class that declares it, or to an ES5 constructor function itself. Where the index names a *class* at the token the class declares no constructor and the key names the base's, so nothing is drawn and the refusal is counted (C-168 narrowed; xmpp.js 21, Preact 1, Express 0). What is left of that entry is `super(…)` (the index emits nothing at the token), a JSX tag of a class component (Preact 377 of its 570 `static→class` misses, every one a tag naming a test-body local class the facts carry no reference for; the other 193 are `super`), and a token the index does not resolve. All 570 name one class, `src/index.d.ts:144`, which is a Hobbes symbol; at `extends Component` the index names the merged interface at line 119 (`oracle-grading.md` §10.26). Poison check PASS on every cell, 0 falsely confirmed. The fixture `minijs` 8/8, recall 8/11. **cypress-io/github-action 154/154 (100%)**, recall 89.0% (154/173), graded at 0.2.57-beta with its 177-package tree on both sides and again with none: the same rows and the same graph, the key's external pairs 413 against 173 (`oracle-grading.md` §10.24; a thin cell, 76 `calls` edges). **Blueturboguy07/cue 1,005/1,005 (100%)**, recall **61.8%** (1,016/1,644) at 0.2.62-beta — 124 of them `m.f()` calls on a namespace `require` over `module.exports = { f }` (ADR-144) — and 881/881, 54.3% (892/1,644) when graded at 0.2.60-beta with its 276-package tree on both sides and again with none (`oracle-grading.md` §10.27; drawn for size — 478 `calls` edges): the same 881 rows by site, target, caller and tier, the same graph; the key's external pairs 5,198 against 1,620, and three key pairs the tree moved, each a call through a dependency-typed value where Hobbes draws nothing in either arm. 176 of its misses were one shape, a member of a `module.exports = { … }` literal called through a namespace `require`: ADR-144 took the 124 that name a function; the 49 written *in* the literal have no symbol (C-9/C-58). **The other three had no dependencies installed** (Express has no lockfile, Preact's is pnpm's, xmpp.js's refused). A call into a third-party package is stated at module grain only (`imports → ext:<pkg>`) and no key grades it, here or in TypeScript (C-165, corrected at 0.2.58-beta). Neither lane reads a `jsconfig.json` (C-166; on Preact 120 of 22,804 sites resolve differently under its jsconfig, the key alone) |
| **Go** | this repo; **dagger — 19 of its Go modules** (O4); **BurntSushi/toml, gorilla/mux, junegunn/fzf** (2026-08-27/28); **quic-go/quic-go** — drawn at random, binary roots only (2026-09-02) | **Compiler-graded against RTA (oracle lane O2/O4):** this repo 1,278/1,278 semantic call edges confirmed, 0/3 syntactic (2026-08-25), then **1,283/1,283 with no syntactic edge left** after the Go scope veto (2026-08-28, contained); **dagger 9,851/9,851 across 19 modules after the O4 fixes** (type conversions drawn as calls, 40 of 40, fixed the same day; the root module is not graded as one program, H-9); **toml 1,039/1,039** (recall 71.9% at 4 roots), **mux 1,221/1,221** with 3 `abstract` (a call through a package-level func variable, H-18; recall 82.6%), **fzf 2,832/2,832** (recall 40.8% at 5 roots) after the Go scope veto removed 87 syntactic edges that bound a local `assert := func` to a package function (ADR-090's rule, the Go shape). **quic-go 3,766/3,781 lower-bound (99.6%), 15 contradicted, all 15 oracle-grain** — the RTA over the full test program was OOM-killed on this box (the dagger-root shape, H-9), so the cell is graded at 5 *binary* roots without test packages, and every contradiction is a call the test build routes to a `_test.go` method shadowing an embedded `*Conn`'s (scip-go indexes the test build; the oracle did not load it) — **0 hobbes-wrong**; recall 47.5% at those roots, `static→named` 99.6% (3,795/3,812), the 17 static misses in `interface→named` territory; the cell that found ADR-098 (C-71): before the fix 2 of its 6 syntactic edges were wrong, after it none — graded after the fix. **cobra 2,186/2,186 (recall 71.8% at 2 roots), gitleaks 2,266/2,266 with tests (94.9% at 9 roots) and 2,010/2,010 without (98.0% at 2 roots)** — repowise-bench's pinned draws under our key (2026-09-09, ADR-101 § the 1-1; gitleaks' one wrong syntactic edge, a stdlib import path suffix-matched to the repo's `regexp/` package, fixed and regraded the same day). Static named calls drawn ≥ 99.6% on every cell. Contained cells (ADR-092): this repo (2026-08-28), mux, fzf, quic-go, cobra, gitleaks; dagger and toml are host-run records. Calls through interfaces, function values and into closures drawn for none (C-58, `docs/oracle/oracle-misses.md`). The V2.M5 20/20 hand-check is **retired** (2026-08-25, Max): its edges were never named; the oracle replaces it |
...
| **Java** | **jhy/jsoup** (Maven library); **spring-projects/spring-petclinic** (Spring service); **spring-data-elasticsearch** and **Legend-of-Dragoon-Modding/Severed-Chains** — both drawn at random (2026-08-29) | **Compiler-graded against javac's own resolution, with CHA for dispatch (oracle lane O8, ADR-096):** every semantic cell at **100% precision-against-oracle, 0 contradicted** — jsoup 18,627/18,627 (recall 76.2% of 24,630 in-repo pairs), spring-petclinic 356/356 (recall 98.4%), spring-data-elasticsearch 16,050/16,050 (recall 66.4%). **Severed-Chains is the lane-A-only cell**: scip-java's own Gradle injection failed on that build (C-67), so all 10,154 edges are syntactic — and all 10,154 are confirmed, 0 contradicted, at recall 23.5% (C-8's floor, measured). Misses are C-58's Java face, the majority class: `interface→method` recall 57.4–98.7% per cell (the CHA override set), plus anonymous-class members below the symbol floor (C-9). Lane agreement: jsoup 3,417 sites / 0 disagreements, petclinic 36 / 0, spring-data-elasticsearch 3,908 / 2 (two same-named calls on one line). Poison check PASS on every cell, 0 falsely confirmed. Contained (ADR-092) — every cell. **No hand-checked edges**: the fixture `minijava`'s eighteen pairs are hand-computed in the lane's test |
| **C** | **DaveGamble/cJSON** (a library with its vendored Unity tests); **sqliteai/sqlite-vector** — drawn at random (2026-09-12) | **Compiler-graded against clang's own front end, per translation unit over the compile database the ingest derives (oracle lane O9, ADR-110).** clang is also lane B's engine (scip-clang), so the key checks what Hobbes adds above the front end: the moniker decode, the join, lane A's fallback. **cJSON 1,188/1,188 confirmed, 0 contradicted**, every one semantic (the 525 syntactic edges sit in files CMake's defaults do not compile, silent); recall 62.0% (1,190/1,918): `static→function` 1,190/1,190, and every miss (728) a call a macro's expansion makes, Unity's `TEST_ASSERT_*` into `UnityFail` and `UnityAssert*` (723) and cJSON's own `cJSON_SetNumberValue` into `cJSON_SetNumberHelper` (5), which Hobbes draws to the macro (C-131). **sqlite-vector 851/851 (100.0%) since 0.2.8-beta**, when the external veto (ADR-111) removed the 3 wrong edges and no confirmed edge anywhere. At 0.2.5-beta it read **851/854 (99.6%)**: the semantic tier 851/851, and **3 syntactic edges wrong**, a `strcasestr` shim defined in an `#if` arm a glibc build does not take, drawn by lane A's same-file rule where lane B's answer was libc's (C-131, C-138); recall 100% (1,091/1,091), the vendored SQLite amalgamation outside the build and silent (18,502 edges). The `minic` fixture 5/5, recall 7/7. Lane agreement cJSON 1,717 / 0, sqlite-vector 1,084 / 0. Poison check PASS on every cell, 0 falsely confirmed. Contained (ADR-092), every cell. The first draw, a Rust-built BPF tool, had no compile database to derive and was passed over (C-135) |
| **C++** | **fmtlib/fmt** (a header-heavy library with its bundled gtest and gmock); **Taywee/args** — drawn at random (2026-09-14) | **Compiler-graded against clang's own front end, per translation unit over the compile database the ingest derives, each unit under clang++ (oracle lane O10, ADR-113 §3).** clang is also lane B's engine (scip-clang), so the key checks what Hobbes adds above the front end. **fmt 7,012/7,012 (100%) at 0.2.45-beta and unmoved at 0.2.46-beta, strict 99.62% (7,012/7,039; ADR-124), recall 30.4% (15,339/50,524)** — 19 of them the constructions the join's by-name claim hid until ADR-133, 92 of them constructions drawn where scip-clang names the constructor at exactly a construction token, outside a template (ADR-132, C-162; 6,901/6,901, strict 99.61%, recall 30.1% at 0.2.42-beta), 391 of them operators applied by symbol, drawn where scip-clang names one at exactly the token lane A's parse shows and only outside a template (ADR-131, C-146 narrowed; 437 such references inside templates are withheld — no call and, since 0.2.43-beta, no `uses` either — because every row a naive rule added that the key could not judge was there and about 100 of those 161 read wrong by hand). At 0.2.41-beta it read 6,510/6,510 (strict 99.59%, recall 29.1%), twice the edges of 0.2.37-beta's 3,269/3,269 (strict 99.73%, recall 14.5%), since ADR-129 reads the definitions lane A's macro-broken parse loses from scip-clang's own definition rows (1,690 symbols, `declared_by: "scip"`, targets not scopes; C-145 narrowed) and ADR-130's R-arity withholds a call written with more arguments than the index's target can take (40 rows, none confirmed: the 35 wrong `copy` edges the mint would have surfaced and 5 that were standing). Every graded edge semantic — 99.88% at 0.2.33-beta, 99.85% at 0.2.28-beta; 3,254/3,282 (99.1%) at 0.2.22-beta, before the oracle's own H-28, H-29, H-30 and H-31 were all fixed on 2026-09-16. **At 0.2.33-beta every one of the 4 contradictions left was scip-clang's single wrong candidate at a call in a template (C-153, P9); at 0.2.37-beta R-qual withholds them and four more of that shape where the written specialisation contradicts the index (ADR-125: 8 wrong edges removed, 0 right).** The fifth, a call drawn inside an unevaluated `decltype` operand which the compiler never calls, was lifted the same day (C-155, ADR-121): lane A records no site under `sizeof`, `alignof`, `decltype`, `noexcept(..)` or a requires-expression, which took 299 sites and 26 edges off this cell — the contradiction and 25 oracle-silent rows. The oracle's contribution to this cell is zero; its own half of C-155 (the key holds a site under `sizeof`, `noexcept` and `typeid`) is H-32. **The 100% is still flattered by H-30:** its silence rule leaves 27 rows unjudged, three of them known C-153 wrong edges (the two `format_as` rows and `format.h:2387`'s `write`, a same-arity wrong candidate R-arity cannot see; 18 of the 19 rows the mint added read by hand as right), so the strict figure counting them as contradicted is **99.62%** (7,012/7,039; ADR-124), and `docs/comparative/tables.md` carries both. What is left of recall: constructions (8,006 misses, 80% of them at a macro invocation's name — C-131 — and the rest registered in C-162), operators inside a template or a macro's expansion, which are recall only (C-146), the macro classes, and the two rules below (the *caller* of a call written inside a lost definition is C-145's and ADR-134's question, not recall: no key reads a caller). At its first grade (0.2.21-beta) fmt read **96.1%** (3,439/3,577): 36 wrong semantic edges from C's one-target rule meeting overloads, and 74 wrong syntactic edges from lane A's name fallback in files lane B compiled. ADR-113 §2's third amendment fixed both (C-151, C-152). **args 2,567/2,567 (100.0%) at 0.2.44-beta, recall 72.9% (2,595/3,561)** — the held-out cell of ADR-132's rule as of ADR-131's and ADR-129's: 369 construction edges added and 369 confirmed (2,198/2,198 and 62.5% at 0.2.42-beta, where ADR-131's rule added 136 operator edges and 136 confirmed; 2,062/2,062 and 58.6% at 0.2.41-beta, where ADR-129's rule added 67 and 67 confirmed); 1,995/1,995 and 56.4% from 0.2.22-beta, 99.8% at its first grade (the 4 contradictions were the collapse); the misses constructions and member calls. Lane agreement fmt 2,239 / 316, where no disagreement draws an edge (read against the key in the cell record); args 793 / 2. Poison check PASS on every grade, 0 falsely confirmed. Contained (ADR-092), every cell. **No hand-checked edges**: the fixture `cppclang`'s 25 sites are hand-keyed in the lane's test. fmt was graded before its pre-registration existed, args after it (`oracle-grading.md` §10.6) |
...
*Retired from the base, 2026-08-25 (Max):* private-repo-A and
qwen-pathology — a handful of hand-checked edges each, too little
weight to carry a row and confusing beside compiler-graded cells. Their
findings live on where they produced a rule (the code comments and the
register cite them); their rows do not.

The asymmetry is the point of the table, so state it plainly rather than
letting the rows read as peers. Python and TypeScript were proven across
multiple repos of different shapes. **JavaScript's three repos arrived in one session**
(ADR-140, §10.22): a CommonJS library and an ESM/JSX library, named, and
a workspaces monorepo drawn at random — each graded without its
dependencies installed, so a call into a third-party package is outside
the claim (C-165). **Go's hand-verified evidence base was one repo, Hobbes's own** — a
...
**Java's four repos arrived in one session** (ADR-096) and are compiler-graded rather than hand-checked, on the widest first sample any language has had — a library, a framework service and two random draws — but they are four Maven/Gradle repos of ordinary shape: an Android build, a Bazel monorepo, an annotation-processor-heavy service and a Kotlin-mixed tree are all outside the sample, and one of the four could not run lane B at all.
**C's two repos arrived in one session too** (ADR-110): a library with a vendored test framework, chosen, and a random draw, both built by CMake or make with nothing fetched. Autotools, Meson and Bazel roots (C-135) and a kernel-style build are outside the sample.
**C++'s two repos arrived in one session as well** (ADR-113): a header-heavy library with its bundled gtest, chosen, and a header-only parser drawn at random, both built by CMake with nothing fetched. Outside the sample: a build over the per-unit bound (C-149) or whose facts outgrow the Python side's memory (C-150; the helper's own decode streams since 0.2.25-beta, ADR-115), C++ modules, generated code, and a mixed repo's shared headers (C-142). A call in a template can carry scip-clang's own wrong candidate (C-153).
...
larger workspaces, and build layouts unlike the sample's are outside the
claim because they are outside the evidence. Two bodies of evidence sit
deliberately *outside* the table's rows, per P11's own rule that a row
licenses what it measured: **dagger** (ADR-048/049 — Go and TS at
~265,000 detected sites, the cross-unit join's +8,014 semantic edges,
36,703 dual-resolved sites with the disagreements accounted for) carries
**no hand-checked edges** and so licenses no row; and the **SWE-bench
repos** extend the Python row at span/declaration grain only. Both are
recorded in [`extraction-evidence.md`](extraction-evidence.md), the
evidence log behind and beyond this table.
...
The table is current to **2026-09-17**: every compiler-graded cell
stands at 100% precision-against-oracle except one. quic-go reads 99.6%
as a lower bound, every contradiction oracle-grain and 0 hobbes-wrong.
C++'s fmt reads 100% with a strict 99.61% beside it (ADR-124): its
four contradictions, scip-clang's wrong specialisation, are withheld
where the written qualifier contradicts the index (ADR-125), a call
written with more arguments than the index's target takes draws nothing
(ADR-130), and three C-153 rows stay unjudged rather than fixed. Its
recall doubled at 0.2.41-beta (14.5% → 29.1%) on definitions read from
the index where lane A's parse lost them (ADR-129), and rose to 30.1%
(args 58.6% → 62.5%) at 0.2.42-beta on operators drawn as calls outside
templates (ADR-131), to 30.3% (args 72.9%) at 0.2.44-beta on
constructions drawn at the token (ADR-132), and to 30.4% at 0.2.45-beta
when the join's claim went by position (ADR-133). The
call drawn inside an unevaluated operand (C-155) was lifted at
0.2.33-beta (ADR-121). All four of the oracle's own defects on this
cell — H-28, H-29, H-30 and H-31 — were fixed on 2026-09-16 and the
cells regraded; its fifth, H-32, is C-155's own half. C's
sqlite-vector reached 851/851 at 0.2.8-beta, when ADR-111's veto
removed its three syntactic hobbes-wrong edges (C-138); ajv's three
rows closed by ADR-104's abstention, hono's last by C-98's lift
(768/768, 2026-09-10) — and each row names
...
gets graded at scale** (ADR-089, `docs/oracle/oracle-grading.md`,
...
front end (O9, and O10, its C++ face) — and reports **precision-against-oracle (a lower bound — contradictions mostly triage to the oracle's grain; the triage ratio is quoted per cell, A-8) and
...
(ADR-092). Since then O8 (javac with CHA, 2026-08-29, ADR-096), O9
(clang's front end, 2026-09-12, ADR-110) and O10 (its C++ face,
2026-09-14, ADR-113; the cells 2026-09-15) are built and run, contained.
The fixtures (`minigo`, `twomod`, `minits`, `miniapp`, `minirust`,
`minijava`, `cclang` for C and `cppclang` for C++) remain the self-test every run of the
suite lands on.
...
- **Context faults are tagged, never refused.** The agent dir is
  ...
  it `context_fault: true` in the flight log while serving it —
  agent-mapping §4's page-fault signal, the first loss term the
  recorder observes.
...
`seed_source` records which).

**The planner is the requirement-decomposer (ADR-084, built as
ADR-085).** It is the one role that reads the whole request, and its
handoff says *what must become true*, not only where: a
...
`--proposal-in-brief` is the control arm of the removal test (ADR-084
§3), not a default. The verifier reads the same requirement list as a
checklist. The weight follows the role: `--planner-arg` raises the one
comprehension session's turn/token budget alone. What the check cannot
do is registered as **C-57**: ownership is by named file, never by
meaning, and the companion measure — the proposal's imperative
sentences the handoff never mentions, `imperatives_unmentioned` — is
lexical, recorded per plan and in the bench report, and gates nothing.
Replayed over stored handoffs with no model, it flags the sympy
value-eval the 7B planner dropped in every 7B run and keeps it for the
27B's (ADR-085). On the planner path only the units the
...
approval flow. Still parked in `future_additions.md`: path-grain write
enforcement at the mount, the renegotiation re-pin, loss fitting, the
re-ingest flag, sub-module partition grain, the approval marker, and
plans and partition records in the web surface.
(The generative planner above the lexical seeds and the verifier
session, listed here as parked until 2026-08-23, have been built since
ADR-059 — the list was stale; metering is the harness adapter's, ADR-059
phase 3.) The base exists to be run under the benchmark harness
(ADR-052) and corrected from what it gets wrong.
...
first live instance surfaced two things now handled and registered. A
live session needs a route to the model endpoint, the one thing the
...
Two later findings reframed the programme. **SWE-bench Verified is
contaminated** — the 27B reproduced a gold patch verbatim, an
author-only version string included, so C-39's bound became a
demonstration — and honest testing moves to **DeepSWE 1.1**, where the
*model + prompt* baseline runs on a **referenced open harness**:
mini-swe-agent under Pier, Hobbes's aid injected through the harness's
prompt-template seam (`pipeline/scripts/deepswe_*`, ADR-078..081).
**P12 (ADR-082)** then retracted those aided pairs as Hobbes evidence:
one agent given the whole task plus an added prompt is *model + prompt*
whatever its score. `--implement-mode aided` on `hobbes bench run`
(ADR-077 — one free agent given the task, what we can see, and what we
cannot) exists as exactly that labelled arm, never as a Hobbes test —
...
no Bash, `--strict-mcp-config`. It runs in `hobbes-session` beside its
sidecar (ADR-112) and behind the egress allowlist:
- **the session's network:** its own `--internal` network;
- **the sidecar:** a `hobbes-proxy sidecar` container on that network,
  the only writer of the session's records — the flight log, the
  escalation queue and the mail file over one flight stream the proxy
  claims once (`serve --sink`; the progress hook posts its edit lines
  the same way), and the egress log. The doer's HOME is a tmpfs, and
  only `<id>/in/` (the MCP config, the hook's settings) is mounted,
  read-only, so nothing of the session dir is in its reach;
- **the route out:** with `--egress`, the same sidecar joins the
  `hobbes-egress` bridge and tunnels CONNECT to the named hosts alone,
  logging each decision.

An explicit `--network` (the bench's pasta; the owned loop needs it for
its transcript) has no sidecar: the session dir stays mounted read-write
and the proxy keeps its file journal, and the launcher says so in one
line (C-140 in full there).
...
- **Policy engine (Go, single implementation)** — builtin floor → box →
  repo → role → folder → agent merge (the role and agent levels added at
...
- **Sandbox** — Podman rootless, **one image, role-shaped mounts and
  ...
  over the cache root's rw mount — calvin-m0-go §2.3; a generated file
  is regenerated, never applied, and a failing generation is retried
  up to three times with each attempt recorded, C-103), and **a
  session may bind host trees read-only**
  ...
  network, and beside it the sidecar, which tunnels to the named hosts
  alone, every decision in the session's `egress.jsonl` (ADR-107; C-41
  narrowed). Since 0.2.14-beta (ADR-112) the sidecar is also the only
  writer of the session's records: the doer's container mounts no part
  of the session dir read-write (HOME is a tmpfs; `<id>/in/` is read-only),
  and the proxy feeds the sidecar's sink over one flight stream claimed
  once — a second is refused and recorded (C-140 narrowed to a forged
  edit line). `.tfstate` denied at box level;
  `derived/` never committed.
- **Escalation** — parked sessions, Sessions-tab cards, expire-to-deny
  (default 30 min), logged approvals.
- **Flight recorder** — append-only JSONL per session, written by the
  sidecar's sink from a stream the proxy holds (ADR-112; the file
  journal in the proxy's own process serves `--knowledge-only` and a
  `--network` override):
  ...
  The sink's own lines carry `tool: "sink"`, an empty `argv`,
  `policy_rule: "sink"` and a `decision` of `listening`,
  `stream_opened`, `stream_closed` or `stream_refused` (ADR-112): the
  bracket a session's log is read within.
...
**Hobbes 0.2.70-beta** (2026-09-24, ADR-103; beta: graded, not stable; the latest tag `v0.2.10-beta`, the one before it `v0.1.8-beta`; 0.1.9-beta to 0.2.9-beta and 0.2.11-beta to 0.2.70-beta untagged; `CHANGELOG.md` is the
...
| **Oracle-grading lane** (ADR-089) | built, both phases run, **reviewed 2026-08-27** | `bench/oracle`: the call graph graded against RTA / `tsc` / the interpreter / rustc's MIR / javac / clang's front end on this repo, kbet, rust_proj, dagger and the seven-repo loop; every compiler-graded cell at 100% precision but quic-go's 99.6% lower bound, all 15 contradictions the oracle's grain; C++'s fmt read 99.1% at its regrade, 10 of its 28 Hobbes' own through scip-clang (C-153), and reads 100% since ADR-125 with a strict 99.61% beside it at 0.2.42-beta (ADR-124, ADR-129, ADR-130, ADR-131) (C's sqlite-vector read 99.6% from three wrong syntactic edges until ADR-111's veto, 0.2.8-beta: 851/851, §3.8); its own defect log reviewed (`oracle-defect-review.md`) |
...
| **C++** (ADR-113) | lane A and O10 wired 2026-09-14 (0.2.18-beta), lane B deliberate the same day (0.2.19-beta), order-independent (0.2.20-beta), one unit per run (0.2.21-beta); **compiler-graded 2026-09-15** (0.2.22-beta's rules, 0.2.23-beta's row, §3.8) | `cppsource` (tree-sitter-cpp 0.23.4) and scip-clang over C's derived compile database, built through the harness in six dispatched sessions. Graded against clang's front end (O10) on fmtlib/fmt (chosen) and Taywee/args (drawn at random). The first grade (96.1%, 99.8%) found two Hobbes rules the decode and the join needed, ADR-113 §2's third amendment; after them fmt reads 99.1% and args 100%. C-142–C-153 (C-144 lifted; C-149 and C-150 shared with C and every language; C-150 narrowed to the Python side at 0.2.25-beta, ADR-115, when the helper's decode was made to stream) |
| **Calvin harness** (ADR-107; `calvin/calvin-harness.md`) | built 2026-09-12, 0.1.21-beta (the 0.2.0-beta minor); validated by use at 40 sessions (Max, 2026-09-13), a mark passed 2026-09-17 with one false block (closed) and none missed; it stays the way work is done, and the tracker (the table at the end of `docs/calvin/sessions/README.md`) reads 74 of 40 at 0.2.70-beta. The first thirty-one session logs ran through 0.2.28-beta (from a rejected token and the `path` alias to C's lane A and oracle, the progress hook, ADR-111's veto, `list_blind_spots`' directory rollup, C-139's lift, the session's containment, D-r's fix, C-134's registrations, the session tracker, which is the table at the end of `docs/calvin/sessions/README.md`, D-s's fixture fix, ADR-112's sink and launcher, C-133's include record, C-135's compile-database check, `oracle import --lang c`, the foreign converters' macro reading, the doer's model named per checkout, C++'s lane A and oracle, C++'s lane B with every overload a symbol, C/C++ lane B made order-independent, and C/C++ indexed one translation unit per run, C++'s lane B made to answer only where it is sure, after its first oracle cells, and the gate's TS/JS arrow-parameter read, which closed the harness's one false block); the progress hook (0.2.6-beta), the dispatch box (0.2.7-beta), a session mounting only its own dir, with `find`'s executing forms and `xargs` as questions (0.2.11-beta, ADR-107's 2026-09-13 amendment), and verify's worktrees self-contained, with no alternates file, so `git` works in its container (0.2.12-beta, D-r) since; **a session's records leave the doer's container** (0.2.14-beta, ADR-112, two dispatched units): the sidecar container per session is the only writer of the flight log, the escalation queue, the mail file and the egress log, fed over one flight stream the proxy claims once; the doer's HOME is a tmpfs and only `<id>/in/` is mounted, read-only; **the doer's model is the checkout's** (0.2.17-beta, ADR-107's 2026-09-14 amendment): `hobbes dispatch` reads `$HOBBES_DISPATCH_MODEL` when `--model` is not given, and the log's Doer line names it | `hobbes dispatch`: Claude Code in `hobbes-session` beside its sidecar and behind the egress allowlist (`hobbes-session --egress`, `hobbes-proxy sidecar`), `hobbes gate` with a derived map and `hobbes verify` on its diff, one log file per session under `docs/calvin/sessions/`; the keyed rounds (M0, M0-Go, M0-Gate) closed as an approach, their records history; C-41 narrowed, C-124 superseded, C-125–C-129 (C-125 narrowed by the hook), C-140 narrowed to a forged edit line (in full on an explicit `--network`) |
