...
|---|---|---|
| **Python** | this repo (dogfood, continuous — **trace-graded**); + six SWE-bench repos at span/declaration grain (astropy, django, scikit-learn, sphinx, sympy, xarray) | **Trace-graded (oracle lane O6, 2026-08-25):** this repo's Python zone under its own suite — 3,291/3,490 call edges confirmed by the interpreter, 10 suspect (6 hobbes-wrong, all syntactic), 189 unexercised; **recall-against-executed 86.2%, 96.9% on named declarations** (closures and lambdas draw no edge — C-58); a coverage-limited claim, never precision (C-60, `docs/oracle-cells/hobbes-py-2026-08-25.md`). 3,070 call sites measured with per-file resolution coverage (ADR-029); lane agreement 1,789 sites / 0 disagreements; 10/10 sampled narrative claims resolve (M5). The derivation programme (2026-08) hand-checked **spans and declaration sites** on the six benchmark repos (24k–608k detected sites each — every one checked landed exactly); **call edges were not separately sampled there**, so those repos extend this row at span/declaration grain only (evidence log) |
| **TypeScript / JavaScript** | kbet (real Vite+React app), this repo's `web/` | kbet: **compiler-graded (oracle lane O3, 2026-08-25): 630/630 call edges confirmed by the zone's own `tsc`**, recall 633/637 on declared callees and 633/1,529 over every resolved site (local bindings, closures and interface members draw no edge — C-58, C-32); lane agreement 359 sites / 0; 10/10 test mappings (M6); the V2.M3 20/20 hand-check retired 2026-08-25 |
| **Go** | this repo; **dagger — 19 of its Go modules** (O4) | **Compiler-graded (oracle lane O2, 2026-08-25):** 1,278/1,278 semantic call edges confirmed by RTA, 0/3 syntactic; static-call recall 1,280/1,280 at 20 roots; calls through interfaces, function values and into closures are drawn for none of 45 (C-58, `docs/oracle-misses.md`). 216 nodes (V2.M5). The V2.M5 20/20 hand-check is **retired** (2026-08-25, Max): its edges were never named; the oracle replaces it. **dagger, compiler-graded (oracle lane O4, 2026-08-25): 9,816/9,856 edges confirmed across 19 modules (99.6%), the 40 contradictions all one product defect — type conversions drawn as calls; 9,854/9,889 static named calls drawn; C-58/C-59 misses as recorded.** Licenses this repo's Go zone and dagger's 19 graded modules; dagger's root module is not graded as one program (H-9) |
| **Rust** | `rust_proj` (one small crate); **dagger — its `sdk/rust` workspace** (O7) | **Compiler-graded (oracle lane O7, 2026-08-25):** rust_proj 17/17 call edges confirmed by rustc's MIR resolution (ADR-040's 33/33 hand-check counted calls and uses; superseded); dagger `sdk/rust` **3,574/3,574 semantic edges confirmed**, 12 syntactic contradicted (`format!` bound to a local `fn format`, C-7), recall 3,593/3,662 — the misses are code macros and derives wrote (`docs/oracle-misses.md`). Lanes clean at 17 sites (V2.M7) |
