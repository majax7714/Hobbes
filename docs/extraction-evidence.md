...
| Date | Numbers |
|---|---|
| 2026-09-14 (**C-133**, the include record; `47f7`'s code, lane A, host) | **6 `c-includes` records**, with the 305 include edges and 12 external nodes unchanged: `"ProductionCode.h"`/`"ProductionCode2.h"` ambiguous in four of the vendored Unity examples' directories (each example ships its own copy, so the suffix step ties), `example_3/helper`'s `"Types.h"` and six mock headers under `test/expectdata` unmatched. sqlite-vector (`0c2223a`) the same day: **1 record**, `libs`' six platform and generated headers (`"sqlite_cfg.h"`, `"_mingw.h"`, `"windows.h"`, …), with its 163 include edges and 57 external nodes unchanged |
...
class: `docs/oracle/oracle-misses.md`.
...
**Pre-registration graded (`docs/oracle/oracle-grading.md` §10, O2):**
...
**Pre-registration graded, phase 2 (`docs/oracle/oracle-grading.md` §10,
...
| **honojs/hono** @ 97c6fe1f | TS | capture 39.7% of 35,239 — `bun.lock` is no lockfile Hobbes provisions (C-34); the root tsconfig is solution-style, so the key is `tsconfig.build.json` (the oracle's new `--config`): **767/774**, recall 55.2% (774/1,403); the 7 are `static→union-member` (`c.toString()` on a `Child` union drawn to `JSXNode.toString`), the ajv provider shape, now n=2 repos. **Regraded 2026-09-09 (later, ADR-104): 767/768** — six of the seven abstained (C-97), the one left a site lane A cannot type under the solution-style root tsconfig (C-98). **Regraded 2026-09-10 (C-98 lifted): 768/768** — the helper types `src/` under `tsconfig.build.json` and its tests under `tsconfig.spec.json`; 15 `union-member` sites where there were 0, capture 39.6%, lane agreement unchanged. **0.1.6-beta (later): lane B under the same map — 768/768, one edge syntactic → semantic (727 semantic confirmed), lanes 4,336 both-resolved / 1, lane B module edges 1,483 → 1,474 (the unclaimed files' references into `src/`, C-12's shape), root-zone references 27,682 → 37,216** |
...
BurntSushi/toml, gorilla/mux, junegunn/fzf (Go, RTA); BurntSushi/memchr (Rust, MIR); ajv-validator/ajv, cheeriojs/cheerio (TS, the zone's `tsc`); pallets/click (Python, trace). Cells in `docs/oracle/cells/*-2026-08-27.md` with their regrade sections; the loop table in `docs/oracle/oracle-misses.md`.
...
| scikit-learn (25102) | 1,052 nodes, 31,523 call edges; python **53.0%** of 82,674; 1 degradation — the duplicate-symbol row (defect D7, `benchmark/cells/adr085-validation-7b-2026-08-24.md`; **corrected by ADR-091**: the duplicate was *in-repo* — `doc/tutorial/text_analytics/{skeletons,solutions}/` — a legitimate C-28 record, not foreign environment residue; the defect was its Rust wording and `path: "."`, both fixed) |
