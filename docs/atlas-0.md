...
**Step 3 — B1 calibration: not started; needs a decision.** Training is not in the tree (torch is not a dependency of the instruments). This box has no GPU; API spend and Modal compute are off the table (Max, 2026-09-04). A CPU run of one 30M cell on the ~50k-line corpus is estimated at hours (one forward at 15 tokens is 62 ms single-threaded in numpy; a torch CPU trainer with batching would do better), which makes step 3 feasible with no spend if Max wants it that way, and the 60-run grid (§5) days of CPU or a small GPU's afternoon. The order of work is step-gated: step 3 opens on Max's go, and the five readings above should be confirmed or overruled first, since a changed reading changes the world hash.
