...
- **fmt** (C++) reads 100% (7,012/7,012), **strict 99.62%** (7,012/7,039)
  ...
  specialisation (ADR-125), or more arguments than the target can take
  (ADR-130) — Hobbes draws nothing. Three known rows of that provider
  error remain, unjudged rather than fixed, and a provider's error in
...
**Hobbes 0.2.67-beta** (2026-09-21). The Hobbes layer is versioned from here
...
dispatched on 2026-09-12, and seventy-one session logs stand. The
...
constructs, ADR-145, and a decorator's calls, ADR-146 to ADR-148: click's
recall 38.2% → 81.2%, at no contradiction),
...
| [`docs/adr/`](docs/adr/) | ADR-001 to ADR-145 (106 closed as *not taken*) — one per decision the running architecture doesn't make |
...
# sessions, and the knowledge tools all run from it (ADR-092/094, ~3.3 GB)
