...
Both are C-4's remainder, with the injected value's type.
"""
...
def injections(
    ...
    """Every parameter pytest's lookup resolves to one repo fixture.

    Returns ``(injections, counts)``. An injection is
    ``{"from", "to", "path", "line", "name"}`` — the requester's symbol id,
    the fixture's, the requester's file, the parameter's line and the
    parameter — sorted by ``(from, to, line)``. *counts* is
    ``{"drawn", "requesters", "fixtures", "abstained", "usefixtures"}``, or
    ``{}`` on a repo where no fixture is defined and no parameter was looked
    up: nothing was asked there, and a block of zeroes would say otherwise.
    """
    ...
    for module in files:
        ...
        for symbol in facts.symbols:
            requester = (
                is_test_symbol(symbol, quals) or _fixture_decorator(symbol) is not None
            )
